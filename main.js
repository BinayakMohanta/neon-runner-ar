...
function initARScene() {
  ...
  THREEx.ArMarkerControls.updateParameters(arToolkitContext);
  ...
  // Non-marker camera for fallback
    ...
    if (isGameRunning) {
      ...
      for (let i = gameObjects.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          const goldCoinMaterial = collectibles.some((c) => c === obj);
          if (goldCoinMaterial) {
            score += 100;
        ...
        if (obj.position.z > camera2.position.z) {
          markerGroup.remove(obj);
...
function spawnObstacles() {
  ...
  const material = new THREE.MeshBasicMaterial({ color: type.color, wireframe: false });
